        ...
        if (histogram) {
            ...
            if (regions.isEmpty()) {
                if (!queryOptions.containsKey("id") && !queryOptions.containsKey("gene")) {
                    ...
                    return setQueryResponse(translateVariantFileIds(variantMongoDbAdaptor.getAllVariants(queryOptions)));
                }
            } else {
                return setQueryResponse(translateVariantFileIds(variantMongoDbAdaptor.getAllVariantsByRegionList(regions, queryOptions)));
            }
        } else {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
